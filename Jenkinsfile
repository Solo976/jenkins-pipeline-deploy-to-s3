    ...
    stages {
        ...
        stage('Deploy to S3') {
            steps {
                ...
                script {
                    sh "aws s3 cp public/index.html s3://jenkinstf-ec2-static-bucket1a/"
                    sh "aws s3 cp public/error.html s3://jenkinstf-ec2-static-bucket1a/"
                }
            }
        }
        ...
        stage('Set Bucket Policy') {
            steps {
                ...
                script {
                    sh "aws s3api put-bucket-policy --bucket jenkinstf-ec2-static-bucket1a --policy file://policy.json"
                }
            }
        }
    }
